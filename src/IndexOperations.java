...
public class IndexOperations {
	...
	public void termAtATimeQueryAnd(String[] query) throws IOException {
		...
		/**-------- Optimizing by rearranging queries ----------------*/
		
		LinkedList<String> queryList = new LinkedList<String>();
		for(int i=0;i<query.length;i++){
			queryList.add(query[i]);
		}
		/** Sorting queries based on increasing document frequency*/
		Collections.sort(queryList, new QueryComparator());
		String[] optimizedQuery= new String[queryList.size()];
		for(int j=0;j<queryList.size();j++)
			optimizedQuery[j]=queryList.get(j);
		int optimizedComparisons=0;
		PostingsData optimizedPDataZero =Index.getIndex().get(optimizedQuery[0]);
		LinkedList<Posting> optimizedIntermediate=optimizedPDataZero.getPostingsListTAAT();
		for(int i=1;i<optimizedQuery.length;i++){
			PostingsData optimizedPDataI =Index.getIndex().get(optimizedQuery[i]);
			LinkedList<Posting> optimizedANDList= new LinkedList<Posting>();
			LinkedList<Posting> optimizedlistSortedTAAT=optimizedPDataI.getPostingsListTAAT();
			for(int j=0;j<optimizedIntermediate.size();j++){
				for(int k=0;k<optimizedlistSortedTAAT.size();k++){
					optimizedComparisons++;
						if(optimizedIntermediate.get(j).getDocID()==optimizedlistSortedTAAT.get(k).getDocID()){
							optimizedANDList.add(optimizedIntermediate.get(j));
							break;
						}

					}
				}
			optimizedIntermediate=optimizedANDList;
		...
		if(intermediateListTAAT==null){
			...
			bw.newLine();
			bw.write(optimizedComparisons+" comparisons are made with optimization");
			bw.newLine();
			bw.write("Result: ");
			bw.newLine();
		}
	...
	public void termAtATimeQueryOr(String[] query) throws IOException {
		...
		for(int i=0;i<query.length;i++){
			PostingsData postingsDataI =Index.getIndex().get(query[i]);
			LinkedList<Posting> listSortedTAAT= new LinkedList<Posting>();
			if(postingsDataI!=null){
				listSortedTAAT=postingsDataI.getPostingsListTAAT();
			}
			else
				continue;
		...
		LinkedList<String> queryList = new LinkedList<String>();
		for(int i=0;i<query.length;i++){
			if(Index.getIndex().get(query[i])!=null)
				queryList.add(query[i]);
		}
		/** Sorting queries based on decreasing document frequency*/
		Collections.sort(queryList, new QueryComparator());
		String[] optimizedQuery= new String[queryList.size()];
		for(int j=0;j<queryList.size();j++)
			optimizedQuery[j]=queryList.get((queryList.size()-1)-j);
		int optimizedComparisons=0;
		LinkedList<Posting> optimizedORList = new LinkedList<Posting>();
		for(int i=0;i<optimizedQuery.length;i++){
			PostingsData optimizedPDI =Index.getIndex().get(optimizedQuery[i]);
			LinkedList<Posting> optimizedListSorted=optimizedPDI.getPostingsListTAAT();
			if(i==0){
				for(int j=0;j<optimizedListSorted.size();j++){
					optimizedORList.add(optimizedListSorted.get(j));
				}
			}
			else{
				for(int j=0;j<optimizedListSorted.size();j++){
					int flag=0;
					for(int k=0;k<optimizedORList.size();k++){
						optimizedComparisons++;
						if(optimizedListSorted.get(j).getDocID()==optimizedORList.get(k).getDocID()){
							flag=1;
							break;
						}
					}
	...
	public void docAtATimeQueryAnd(String[] query) throws IOException {
		...
		/**Checking if any of the query terms are not in the index */
		for(int i=0;i<query.length;i++){
			PostingsData postingsDataTemp =Index.getIndex().get(query[i]);
			if(postingsDataTemp==null){
				bw.write("Term not found");
				bw.newLine();
				return;
			}
		...
		if(DAATList.size()==1){
			...
			while(pointerList[0]<DAATList.get(0).size()){
				...
				for(int j=1;j<DAATList.size();j++){
					...
					/** If docID of current list is still less than that of the first postings list, it implies
					 * that there are no more documents on the next list with greater docID than the first list*/
					 ...
					 * the current list*/
						...
						listPresent=false;
						countComparisons++;
						while(docFirst<docCurrent && endofList[0]==false){
							pointerList[0]++;

							/** While not the end of the first list, update the first docID*/
							if(pointerList[0]==DAATList.get(0).size()){
								endofList[0]=true;
							}
							else
	...
	public void docAtATimeQueryOR(String[] query) throws IOException {
		...
		LinkedList<LinkedList<Posting>> DAATList = new LinkedList<LinkedList<Posting>>();
		for(int i=0;i<query.length;i++){
			PostingsData postingsSortedDAATTemp=Index.getIndex().get(query[i]);
			if(postingsSortedDAATTemp!=null){
				
				DAATList.add(i, postingsSortedDAATTemp.getPostingsListDAAT());
			}
		}
